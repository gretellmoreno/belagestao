...
export default function AppointmentForm({ onClose, isEditing: isEditingProp = false, appointmentId }: AppointmentFormProps) {
  ...
  }, [isEditing, appointmentId, professionals, initialFormState, clients]); // Removi formData das dependências
  ...
  // Refatorar este useEffect para evitar loops de renderização
  useEffect(() => {
    // Manter este useEffect somente para carregamento inicial
    loadData();
        ...
        try {
          ...
          if (appointment.appointment_services && Array.isArray(appointment.appointment_services)) {
            duration = appointment.appointment_services.reduce((total: number, service: any) => {
              const serviceTime = service.custom_time || (service.service?.estimated_time || 30);
          ...
          for (let currentMinute = startMinutes; currentMinute < endMinutes; currentMinute += 15) {
            ...
            if (currentMinute + 15 < endMinutes) {  // Só marcar intermediários se não for o último slot
              for (let i = 1; i < 4; i++) {
                ...
                if (intermediateHour >= 23) continue;
  ...
  }, [showDateTimeModal, tempDate, getOccupiedTimeSlots, formData.professional_id]);

  // Função para limpar o cache e recarregar os slots
  const refreshTimeSlots = useCallback(() => {
    setOccupiedSlotsCache({}); // Limpar todo o cache
    if (showDateTimeModal) {
      const dateStr = format(tempDate, 'yyyy-MM-dd');
      getOccupiedTimeSlots(dateStr, formData.professional_id);
    }
  }, [showDateTimeModal, tempDate, formData.professional_id, getOccupiedTimeSlots]);
  ...
  }, [showDateTimeModal, refreshTimeSlots]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Formatar os serviços no formato correto esperado pelo AppointmentContext
      const formattedServices = formData.services.map(service => ({
        service_id: service.id,
        custom_time: formData.custom_times?.[service.id] || service.estimated_time,
      ...
      const appointmentData = {
        ...
        time: formData.time,
        notes: formData.notes,
        status: 'agendado', // Usar o status correto em português
        _selectedServices: formattedServices // Enviar os serviços no formato correto
      };
      ...
      if (isEditing && appointmentId) {
        ...
        // Após criar o agendamento, buscar os dados completos incluindo appointment_services
        if (appointmentResult && appointmentResult.id) {
          try {
            console.log('Buscando dados completos do agendamento recém-criado:', appointmentResult.id);
            ...
            if (fetchError) {
              ...
              console.log('Dados completos do agendamento obtidos:', completeAppointment);
              
              // Calcular duração total a partir dos serviços
              const totalDuration = completeAppointment.appointment_services?.reduce(
                (total, service) => {
                  // Acessar estimated_time de forma segura com tipagem correta
                  const serviceTime = typeof service.custom_time === 'number' && service.custom_time > 0 
                    ? service.custom_time 
                    : (service.service && typeof service.service === 'object' && 'estimated_time' in service.service && typeof service.service.estimated_time === 'number'
                        ? service.service.estimated_time 
                        : 30);
                  return total + serviceTime;
                }, 0
              ) || 30;
              
              // Criar objeto de agendamento completo para atualização otimista da UI
              const completeData = {
                ...completeAppointment,
                professional_name: professionals.find(p => p.id === completeAppointment.professional_id)?.name || 'Profissional',
                total_value: formData.services.reduce((sum, service) => sum + service.price, 0),
                // Adicionar duração calculada
                duration: totalDuration
              };
              
              // Disparar evento de atualização com dados completos (igual ao fluxo de clique)
              const appointmentUpdatedEvent = new CustomEvent('appointmentUpdated', { 
                detail: { 
                  id: appointmentResult.id,
                  date: formData.date,
                  status: 'agendado',
                  forceRefresh: true, // Forçar refresh para garantir que os serviços sejam carregados
                  appointmentData: completeData // Dados completos para atualização otimista
                } 
              });
              window.dispatchEvent(appointmentUpdatedEvent);
            }
          } catch (err) {
            console.error('Erro ao processar dados completos do agendamento:', err);
          }
        }
      }
  ...
  // Refatorando o allTimeSlotsWithStatus para evitar loops
  const allTimeSlotsWithStatus = useMemo(() => {
    ...
    console.log(`Calculando slots disponíveis com: prof=${professional_id}, data=${dateStr}`);
    
    // Duração total do serviço em minutos - usar uma versão simplificada para evitar loops
    const serviceDuration = formData.services.reduce((total, service) => {
      const customTime = formData.custom_times?.[service.id] || 0;
      return total + (customTime > 0 ? customTime : service.estimated_time);
    }, 0);
    
    console.log(`Duração total do serviço: ${serviceDuration} minutos`);
  ...
  }, [allTimeSlots, formData.professional_id, tempDate, occupiedSlotsCache, formData.services, formData.custom_times]);
  ...
  // Localizar e modificar a parte onde o evento appointmentUpdated é disparado, garantindo
  // que inclua o status 'finalizado' e a data do atendimento quando um atendimento for finalizado

  // Procurando pela função que chama finalize_appointment
  const finalizeAppointment = async (id: string, paymentMethodId?: string, discountPaymentFee: boolean = false) => {
    try {
      setLoading(true);
      ...
      console.log(`[FINALIZAÇÃO] Iniciando finalização do agendamento ${id}`);
      ...
      const { data: rpcResult, error: rpcError } = await supabase.rpc('finalize_appointment', {
        payload: {
          p_appointment_id: id,
      ...
      if (rpcError) {
        console.error("Erro ao finalizar atendimento:", rpcError);
        // A função RPC agora pode lançar erro se o p_appointment_id não for válido
        const errorMessage = rpcError.message || "Erro desconhecido";
        toast.error(`Não foi possível finalizar o atendimento: ${errorMessage}`);
        return false;
      }
      
      console.log("Atendimento finalizado com sucesso:", rpcResult);
      
      // Verificar se o resultado indica sucesso
      if (rpcResult && rpcResult.success === false) {
        console.error("RPC retornou sucesso = false:", rpcResult.message);
        toast.error(`Erro ao finalizar: ${rpcResult.message || "Erro desconhecido"}`);
        return false;
      }
      
      // Obter a data do atendimento para o evento
      const appointmentDate = formData.date || getCurrentDateLocal();
      
      // Criar e disparar o evento de atendimento atualizado com as informações necessárias
      const appointmentUpdatedEvent = new CustomEvent('appointmentUpdated', {
        detail: {
          id: id,
          status: 'finalizado',
      ...
      // Disparar o evento para que o relatório financeiro seja atualizado
      window.dispatchEvent(appointmentUpdatedEvent);
                ...
                <div className="grid grid-cols-4 gap-2">
                  {loadingTimeSlots ? (
                    ...
                    allTimeSlotsWithStatus.map((slotInfo) => {
                      ...
                      const now = new Date();
                      // 🔧 CORREÇÃO CRÍTICA DE FUSO HORÁRIO: Usar formatDateToLocal em vez de toISOString().split('T')[0]
                      // Isso evita que agendamentos desapareçam às 21h devido à conversão incorreta para UTC
                      const currentDate = formatDateToLocal(now);
