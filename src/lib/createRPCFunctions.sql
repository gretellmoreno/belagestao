...
BEGIN
    ...
    WHERE id = finalize_appointment._appointment_id; -- Use o nome da função como prefixo
    ...
    WITH updated_services AS (
        ...
        WHERE 
            as_update.appointment_id = finalize_appointment._appointment_id -- Use o nome da função como prefixo
            AND a.id = finalize_appointment._appointment_id -- Use o nome da função como prefixo
        RETURNING 
            as_update.*
    )
    ...
    SELECT 
        json_build_object(
            'success', true,
            'appointment_id', finalize_appointment._appointment_id, -- Use o nome da função como prefixo
            'status', 'finalizado',
