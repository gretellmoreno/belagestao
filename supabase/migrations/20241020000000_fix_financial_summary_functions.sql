-- Migração para corrigir as funções de resumo financeiro
-- Data: 2024-10-20

-- Corrige os problemas nas funções financeiras utilizando updated_at em vez de created_at
-- e garantindo que os campos net_value estão sendo usados corretamente

-- Remover as funções existentes
DROP FUNCTION IF EXISTS public.get_services_summary(TIMESTAMP, TIMESTAMP);
DROP FUNCTION IF EXISTS public.get_products_summary(TIMESTAMP, TIMESTAMP);
DROP FUNCTION IF EXISTS public.get_payment_methods_summary(TIMESTAMP, TIMESTAMP);
DROP FUNCTION IF EXISTS public.get_professionals_profit_summary(TIMESTAMP, TIMESTAMP);
DROP FUNCTION IF EXISTS public.get_financial_summary(TIMESTAMP, TIMESTAMP);

-- Criar função RPC corrigida para obter resumo de serviços
...
-- Criar função RPC corrigida para obter resumo de produtos
...
-- Criar função RPC corrigida para obter resumo por método de pagamento
...
-- Criar função RPC corrigida para obter lucro por profissional
...
-- Função principal corrigida para obter o resumo financeiro completo
